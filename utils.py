...
def test_model(model, test_loader, device, stats, max_evals=200000):
    ...
    for _, (images, labels) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            lgt_lin, lgt_mlp, lgt_res5, lgt_res7 = res_dict['class']
        # check classification accuracy
        correct_lin += get_correct_count(lgt_lin, labels)
        correct_mlp += get_correct_count(lgt_mlp, labels)
        correct_res5 += get_correct_count(lgt_res5, labels)
        correct_res7 += get_correct_count(lgt_res7, labels)

        total += labels.size(0)
    acc_lin = correct_lin / total    
    acc_mlp = correct_mlp / total
    acc_res5 = correct_res5 / total
    acc_res7 = correct_res7 / total

    model.train()
    # record stats in the provided stat tracker
    stats.update('test_lin', acc_lin, n=1)
    stats.update('test_mlp', acc_mlp, n=1)
    stats.update('test_res5', acc_res5, n=1)
    stats.update('test_res7', acc_res7, n=1)
